...
function health(target) {
    ...
    console.log(clc.red(`  здоровье:${bot.health} `), clc.yellow(`насыщение:${bot.food} `), clc.blue(`позиция  x: ${bot.entity.position.x} y: ${bot.entity.position.y} z: ${bot.entity.position.z} `))
}
function examination(username, message) {
    ...
    switch (message.split(" ")[0]){
        case "goto": goto(message, target)
            break;
        case "come": come(username)
            break;
        case "stop": stop(username)
            break;
        case "health": health()
            break;
    }
...
app.post("/", urlencodedParser, function (request, response) {
    ...
    if (request.body.mes !== undefined) {
        if (request.body.name === bot.username) {
            if (request.body.mes.split('')[0] === '/') {examination(request.body.name, request.body.mes.split('').slice(1).join(''))}
            else {bot.chat(request.body.mes)}
